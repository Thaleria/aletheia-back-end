...
class AzureLLMClient(LLMClientInterface):
    ...
    async def rag_llm_call(self, query: str, context: str, prompt: str) -> str:
        """Calls an Azure Chat OpenAI endpoint with a given message, context
        and prompt.
        ...
        Args:
            ...
            prompt (str): A string template for the prompt to be sent to the
                LLM.
...
def get_azure_llm_client(temperature: float = 0.0, max_tokens: int = 5000,
    ...
    Returns:
        LLMClientInterface: An initialized LLM client object, adhering to the
            `LLMClientInterface`.
    """
    return AzureLLMClient(temperature=temperature,
                          max_tokens=max_tokens,
                          timeout=timeout)
